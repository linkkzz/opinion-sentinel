...
采集流程（复刻 MediaCrawler kuaishou/core.py:69-190 混合模式）：
  ...
  6. block 风控时：goto 首页 + sleep(20) + update_cookies 恢复（复刻 ks/core.py:289-303）
...
class KuaishouAdapter:
            ...
            try:
                ...
                while len(results) < limit and page_count < max_pages:
                    try:
                        ...
                        # block 恢复：goto 首页 + sleep(20) + update_cookies（复刻 ks/core.py:289-303）
    ...
    def _feed_to_raw(feed: dict, keyword: str) -> Optional[RawCollectedItem]:
        """feed → RawCollectedItem。字段映射复刻 store/kuaishou/__init__.py:60-76。"""
