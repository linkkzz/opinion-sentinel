...
复刻 MediaCrawler 的登录与 Cookie 管理策略（weibo/login.py、kuaishou/login.py、
crawler_util.find_login_qrcode），改为 in-process 调用，通过 SSE 把二维码推给前端。

核心设计（对齐 MediaCrawler）：
  - MediaCrawler 用 show_qrcode(PIL 弹图) 作为唯一扫码入口；我们用前端 Modal
...
async def _extract_qrcode_base64(page: Page, selector: str) -> str:
    """从页面提取二维码图片 base64。复刻 crawler_util.find_login_qrcode。

    用 get_attribute("src") 替代 get_property("src")——后者在新版 playwright
    返回 JSHandle 而非字符串值。
    """
    try:
...
async def _check_logged_in(context: BrowserContext, login_cookie_key: str) -> bool:
    """轮询登录态：检查指定 cookie 是否出现。复刻 weibo/login.py:69-83。"""
...
async def _goto_and_maybe_convert(
    ...
    if config["need_mobile_conversion"]:
        # 复刻 weibo/core.py:114-121：goto mobile 首页同步 SSO cookie 到 mobile 域
...
async def refresh_cookie(platform: str, account_id: int) -> bool:
    ...
    复刻 MediaCrawler weibo 432 降级（client.py:89-91 goto+update_cookies）与
    ...
    async with lock:
        ...
        async with async_playwright() as playwright:
            ...
            page = await context.new_page()
            try:
                # 注入旧 Cookie（复刻 weibo/login.py:124-132 login_by_cookies）
                if old_cookie_str:
