...
# 平台纯 HTTP 采集层共享工具。
#
# 本模块基于 MediaCrawler（https://github.com/NanmiCoder/MediaCrawler）二开改造，
# 保留其 NON-COMMERCIAL LEARNING LICENSE 1.1 许可声明，去除 playwright / 代理池 /
# 模块级 config 等运行时耦合，并补齐请求头、UA 池等反风控短板，以便在 collector
# 进程内以纯 HTTP + Cookie 注入方式调用。
#
# 原始版权声明（MediaCrawler）：
# Copyright (c) 2025 relakkes@gmail.com
# Licensed under NON-COMMERCIAL LEARNING LICENSE 1.1
# 使用者应遵守：不得商用；遵守目标平台条款与 robots.txt；不得大规模爬取或干扰平台；
# 合理控制请求频率；不得用于非法用途。详见项目根目录 LICENSE。
...
# PC Chrome UA 池（Win/Mac/Linux，版本 109-123）。比 MediaCrawler 原 20 条略精简但够用。
_PC_UA_LIST = [
...
# 移动端 UA 池（iPhone / Android）。原 MediaCrawler 仅 1 条，此处扩充降低指纹聚类。
_MOBILE_UA_LIST = [
...
# 复刻 MediaCrawler weibo/client.py:84-92 的 goto+update_cookies 432 恢复机制。
# ---------------------------------------------------------------------------

# stealth.js 路径（从 MediaCrawler libs/ 复制，注入反自动化检测）
...
async def update_cookies_from_context(
    ...
    复刻 MediaCrawler crawler_util.convert_browser_context_cookies + 各 client.update_cookies。
...
async def launch_collection_browser(playwright, *, headless: bool = True):
    ...
    复刻 MediaCrawler 标准模式：launch(channel="chrome") + new_context + add_init_script(stealth.js)。
    ...
    # 若文件不存在（如 git clone 后未复制），尝试从 MediaCrawler 自动复制
    _ensure_stealth_js()
...
def _ensure_stealth_js() -> None:
    """stealth.js 不存在时尝试从 MediaCrawler 复制（部署环境需手动放置）。"""
    if _STEALTH_JS_PATH.exists():
        return
    # 尝试常见路径
    candidates = [
        Path.home() / "PyCharmProject" / "MediaCrawler" / "libs" / "stealth.min.js",
        Path("/app/MediaCrawler/libs/stealth.min.js"),  # Docker
    ]
