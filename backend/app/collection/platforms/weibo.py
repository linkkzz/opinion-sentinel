# -*- coding: utf-8 -*-
# 基于 MediaCrawler 二开的微博采集 Client。
#
...
#   需遵守目标平台条款与 robots.txt；不得大规模爬取或干扰平台；合理控频；不得非法使用）。
#
# 二开改动：
#   - 混合采集模式：httpx 发业务请求 + 可选 playwright_page 做 432 风控恢复
#   - 432 恢复复刻 weibo/client.py:84-92：goto m.weibo.cn + sleep(2) + update_cookies → 重试
#   - 无 playwright_page 时抛 DataFetchError 由上层 mark_pending_refresh 刷新
#   - 补全 Accept / Accept-Language / Accept-Encoding 请求头，扩充移动端 UA 池
#   - 保留 @retry(5,3s) 翻页级重试（复刻 weibo/client.py:72）
...
class WeiboClient:
    ...
    :param playwright_page: 可选，传入后 432 风控时自动 goto+update_cookies 恢复。
        复刻 MediaCrawler weibo/client.py:84-92 的恢复机制。
    """
    ...
    async def update_cookies(self, context, urls: Optional[List[str]] = None) -> None:
        """从浏览器 context 同步最新 Cookie 到 httpx headers。复刻 client.update_cookies。"""
    ...
    async def request(self, method: str, url: str, **kwargs) -> Union[Dict, httpx.Response]:
        ...
        try:
            ...
            if self.playwright_page:
                # 复刻 weibo/client.py:84-92：goto 首页 + sleep(2) + update_cookies → 触发重试
    ...
    async def pong(self) -> bool:
        """校验登录态是否有效。复刻 weibo/client.py:118-132。"""
        try:
    ...
    async def get_note_info_by_id(self, note_id: str) -> Dict:
        """获取长微博全文（解析详情页 $render_data）。复刻 weibo/client.py:258-278。"""
